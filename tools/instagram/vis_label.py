    ...
    for i, data in enumerate(data_loader):
        ...
        for vis_format in args.gt_format:
            if vis_format == 'se_pts':
                ...
                plt.axis('off')
                gt_vectors_instance = gt_bboxes_3d[0].fixed_distance_sampled_points
                for gt_vector_instance, gt_label_3d in zip(gt_vectors_instance, gt_labels_3d[0]):
                    pts, pts_num = gt_vector_instance['pts'], gt_vector_instance['num_pts']
                    pts = pts[:pts_num]
                    x = np.array([pt[0] for pt in pts])
                    y = np.array([pt[1] for pt in pts])
                ...
                gt_fixeddist_map_path = osp.join(sample_dir, 'GT_vector_pts_MAP.png')
