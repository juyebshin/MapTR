...
def parse_args():
    ...
    parser.add_argument('--score-thresh', default=0.1, type=float, help='samples to visualize')
    parser.add_argument(
...
def main():
    ...
    colors_plt = ['tab:red', 'tab:blue', 'tab:green'] # 'orange', 'b', 'g'
    ...
    for i, data in enumerate(data_loader):
        ...
        for vis_format in args.gt_format:
            if vis_format == 'se_pts':
                ...
                plt.axis('off')
                gt_vectors = map_dict['gt_vectors']
                for gt_vector in gt_vectors:
                    pts, pts_num, line_type = gt_vector['pts'], gt_vector['pts_num'], gt_vector['type']
                    pts = pts[:pts_num]
                    x = np.array([pt[0] for pt in pts])
                    y = np.array([pt[1] for pt in pts])
                    
                    plt.plot(x, y, color=colors_plt[line_type],linewidth=1,alpha=0.8,zorder=-1) # linewidth=1
                    plt.scatter(x, y, color=colors_plt[line_type],s=2,alpha=0.8,zorder=-1)
                plt.imshow(car_img, extent=[-1.2, 1.2, -1.5, 1.5])
